...
export const PrinterInfo = ({ printer }: { printer: IPrinter }) => (
  <Box sx={{ display: 'flex', alignItems: 'center', width: '100%', gap: 5, flexDirection: { xs: 'column', sm: 'row' } }}>
    <img
    ...
    <Box>
      ...
      >
        ...
        >
          ...
          Max Speed: {printer.maxPrintSpeed_mm_s ? `${printer.maxPrintSpeed_mm_s} mm/s` : 'N/A'}
        </Typography>
        <Typography
          variant="body2"
          sx={{ display: 'flex', alignItems: 'center' }}
        >
          ...
          Nozzle: {printer.nozzleDiameter_mm ? `${printer.nozzleDiameter_mm} mm` : 'N/A'}
        </Typography>
        <Typography
          variant="body2"
        ...
        >
          ...
          Materials: {printer.materials && printer.materials.length > 0
        ...
        >
          ...
          Colors: {printer.colors && printer.colors.length > 0
            ? (
              <Box sx={{ display: 'flex', gap: 1, ml: 1, flexWrap: 'wrap' }}>
                {printer.colors.map((col) => (
                  <Box key={col.name} sx={{ display: 'flex', alignItems: 'center' }}>
                    <Box 
                      sx={{ 
                        width: 16, 
                        height: 16, 
