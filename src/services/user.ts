...
export const sendUpdateEmailVerificationService = async (
  new_email: string
): Promise<any> => {
  try {
    const response = await api.post(`/send-update-email-verification`, {
      new_email,
    });
...
export const updateVerifyUserEmailService = async (
  new_email: string,
  token: string
): Promise<any> => {
  try {
    const response = await api.post(`/update-verify-user-email`, {
      new_email,
      token,
    });
    return response;
