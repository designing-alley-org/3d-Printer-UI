...
export const getUserChatService = async (ticketId: string): Promise<ChatMessage[]> => {
  try {
    // Using dummy data for now - replace with actual API call
    // const response = await api.get(`/get-user-chat/${ticketId}`);
    // return returnResponse(response);
    
    // Simulate API call delay
    await new Promise(resolve => setTimeout(resolve, 800));
    
    // Return chat messages for the specific ticket or empty array
    return dummyChatData[ticketId] || [];
  } catch (error) {
    throw error;
  }
};
